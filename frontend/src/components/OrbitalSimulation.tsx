...
function EarthSystem({
  ...
  useFrame(() => {
    if (earthOrbit.current)
      if (!paused) {
        earthOrbit.current.rotation.y += earthSpeed;
      }

    if (moonOrbit.current)
      if (!paused) {
        moonOrbit.current.rotation.y += moonSpeed;
      }
  });
  ...
  return (
    <>
      ...
      <group ref={earthOrbit}>
        ...
        <Text position={[4, 1, 0]} fontSize={0.2} color="white">
          Earth
        </Text>

        {/* Moon Orbit */}
        <group ref={moonOrbit} position={[4, 0, 0]}>
          <OrbitRing radius={1} />
          <mesh position={[1, 0, 0]}>
            <sphereGeometry args={[0.2, 32, 32]} />
            <meshStandardMaterial color="white" />
          </mesh>
          <Text position={[1, 0.5, 0]} fontSize={0.15} color="white">
            Moon
          </Text>
        </group>
      </group>
...
export default function OrbitalSimulation() {
  const [earthSpeed, setEarthSpeed] = useState(0.01);

  const [moonSpeed, setMoonSpeed] = useState(0.04);
      ...
      >
        ...
        <button
          onClick={() => {
            ...
            controls.object.position.set(0, 6, 12);
        ...
        >
          ...
          <p>
            This simulation demonstrates hierarchical orbital motion using
